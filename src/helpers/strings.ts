...
/cancel - всегда вернёт в главное меню`,
  ...
  professions: {
    generalInfo: `Всего есть 3 роли: катальщики, протиральщики и рекламщики. О какой хочешь узнать?`,
...
- если будет 5 партия, то перед ней надо один мяч принести к ставленнику секретаря ― так положено
    ...
    advertisers: {
      categoryBtn: 'Рекламщики 🎨',
      rules__html: `
<b>Рекламщики 🎨</b> подготавливают поле к матчу. Они приходят за несколько часов до матча и расставляют столы для судей, устанавливают щиты с рекламой по периметру игрового поля.
        `,
    },
...
Во время перерывов и между партиями игроки пьют воду. Как только они разошлись от места и пошли на расстановку надо место водопоя протереть, чтобы луж не было.    
  ...
  membersList: {
    selectEvent: 'Список участников какого мероприятия выдать?',
    emptyEvent: 'Нет подтверждённых участников',
  },
  errors: {
